...
1. Copy `pglite.wasm` and `pglite.data` from `node_modules/@simthis/pglite/dist/` to your public/build directory so your web server can serve them.
...
When using [Next.js](https://nextjs.org/), make sure to add `@simthis/pglite` to the `transpilePackages` array in `next.config.js`:

```js
const nextConfig = {
  swcMinify: false,
  transpilePackages: [
    '@simthis/pglite-react', // Optional
    '@simthis/pglite',
  ],
}
